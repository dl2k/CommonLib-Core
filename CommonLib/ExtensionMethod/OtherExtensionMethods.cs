    ...
    public static class OtherExtensionMethods
    {
        ...
        public static T DataTableToEntity<T>(this DataTable table) where T : new()
        {
            ...
            try
            {
                ...
                {
                    var ignoreAttr = item.GetCustomAttributes(typeof(IgnoreDBColumnAttribute), true);
                    if (ignoreAttr.Length > 0)
                        continue;

                    var dbColumnAttributeList = item.GetCustomAttributes(typeof(DBColumnAttribute), true);
                    string columnName = string.Empty;
                    if (dbColumnAttributeList != null && dbColumnAttributeList.Length > 0 && dbColumnAttributeList[0] != null)
                    {
                        columnName = (dbColumnAttributeList[0] as DBColumnAttribute).DBColumnName;
                    }
                    ...
                    if (DBNull.Value == row[columnName]) continue;
                    item.SetValue(entity, item.PropertyType.IsGenericType ? Convert.ChangeType(row[columnName], item.PropertyType.GetGenericArguments()[0]) : Convert.ChangeType(row[columnName], item.PropertyType), null);
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataTable ToDataTable<T>(this IEnumerable<T> list)
        {
            ...
            try
            {
                ...
                foreach (var item in list)
                {
                    ...
                    pList.ForEach(p =>
                    {
                        var ignoreAttr = p.GetCustomAttributes(typeof(IgnoreDBColumnAttribute), true);
                        if (ignoreAttr.Length == 0)
                        {
                            var dbColumnAttributeList = p.GetCustomAttributes(typeof(DBColumnAttribute), true);
                            string columnName = string.Empty;
                            if (dbColumnAttributeList != null && dbColumnAttributeList.Length > 0 && dbColumnAttributeList[0] != null)
                            {
                                columnName = (dbColumnAttributeList[0] as DBColumnAttribute).DBColumnName;
        ...
        public static List<T> DataTableToList<T>(this DataTable dt) where T : class, new()
        {
            ...
            try
            {
                ...
                Type t = typeof(T);
                Array.ForEach<PropertyInfo>(t.GetProperties(), p => { prlist.Add(p); if (!dt.Columns.Contains(p.Name)) dt.Columns.Add(p.Name, p.PropertyType); });
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    prlist.ForEach(p =>
                    {
                        ...
                        if (ignoreAttr.Length == 0)
                        {
                            ...
                            if (row[columnName] != DBNull.Value) p.SetValue(ob, row[columnName], null);
                        }
                    });
                    oblist.Add(ob);
                }
        ...
        /// <summary>    
        /// 单列DataTable 转换为List 集合    
        /// </summary>    
        /// <typeparam name="T">类型</typeparam>    
        /// <param name="dt">DataTable</param>    
        /// <returns>集合</returns>    
        public static List<T> SingleCloumnDataTableToList<T>(this DataTable dt)
        {
            List<T> oblist = new List<T>();
            try
            {
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public static List<PropertyInfo> GetAllProptyFromEntity(Type t)
        {
            if (t == null)
            ...
            if (ps == null || ps.Length <= 0)
                return new List<PropertyInfo>();
            return new List<PropertyInfo>(ps);
        }
        #endregion
        ...
        /// <summary>
        /// 将数值型数组转换成In后面的条件(一般用于ID的IN条件)
        /// </summary>
        /// <param name="pCaller">字符串数组</param>
        /// <returns>不带括号的In查询条件</returns>
        public static string ToSqlInInt(this int[] pCaller)
        {
            string conditionIds = string.Empty;
            foreach (var conditionIdItem in pCaller)
            {
                conditionIds += "'" + conditionIdItem.ToString().Replace("'", "''") + "',";
            }
            if (conditionIds.Length > 0)
                conditionIds = conditionIds.Substring(0, conditionIds.Length - 1);
            return conditionIds;

...
namespace CommonLib.Utils
{
    ...
    public class Logger
    {
        ...
        public static void WriteLog(string info, LogLevel level, Exception ex)
        {
            ...
            data.Level = level;
            data.Exception = ex;
            data.CreateDateTime = CommonUtils.GetNow();
        ...
        private static string GetWriteLogPath()
        {
            ...
            //else
            {
                path = System.Threading.Thread.GetDomain().BaseDirectory + string.Format(@"\log\{0}\{1}\{2}\", CommonUtils.GetNow().Year, CommonUtils.GetNow().Month, CommonUtils.GetNow().Day);
            }
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            path = path + string.Format("Log-{0}.log", CommonUtils.GetNow().ToString("HH"));
